...
#define TURING_QDSP6SS_RET_CFG		0x0230001c
 ...
 * stale contents block the host from talking to the CDSP.
 */
...
static TEE_Result cdsp0_fw_start(struct qcom_pas_data *data)
{
	...
	/* Keep the retention registers through later core resets */
	io_write32(base + TURING_QDSP6SS_RET_CFG, 0);
	dsb();
